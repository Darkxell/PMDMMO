	...
	public void render(Graphics2D g, int width, int height)
	{
		...
		for (int x = xStart; x <= xStart + width / TILE_SIZE + 2; ++x)
			for (int y = yStart; y <= yStart + height / TILE_SIZE + 2; ++y)
			{
				...
				else if (tile.trapRevealed) sprite = SpriteSets.dungeonCommon.trap(tile.trap.id);
				else if (tile.type() == TileType.STAIR) sprite = SpriteSets.dungeonCommon.stairs(this.floor.dungeon.dungeon().direction);
				else if (tile.type() == TileType.WARP_ZONE) sprite = SpriteSets.dungeonCommon.warp();
