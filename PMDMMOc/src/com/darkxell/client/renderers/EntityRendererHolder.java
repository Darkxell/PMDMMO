...
public class EntityRendererHolder<T> extends AbstractRenderer {
    ...
    /**
     * True if the sorted list should be updated when called. Security in case registered() is called between update()
     * and listRenderers().
     */
    ...
    public AbstractRenderer getRenderer(T object) {
        if (this.renderers.containsKey(object))
    ...
    public AbstractRenderer register(T object, AbstractRenderer renderer) {
        if (this.renderers.containsKey(object))
            return this.getRenderer(object);
