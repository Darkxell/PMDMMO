...
public class OnFirstPokemonDraw {
    ...
    public static boolean isNotVisible(DungeonPokemon pokemon) {
        ...
        Rectangle screen = new Rectangle(camera.x, camera.y, PrincipalMainState.displayWidth,
                PrincipalMainState.displayHeight);
        Rectangle spriteDim = new Rectangle((int) (renderer.drawX() - sprite.getWidth() / 2 + frame.spriteX),
                (int) (renderer.drawY() - sprite.getHeight() / 2 + frame.spriteY), sprite.getWidth(),
                sprite.getHeight());
    ...
    public static void reset() {
        seen.removeIf(t -> !Persistence.player.isAlly(t));
    }
