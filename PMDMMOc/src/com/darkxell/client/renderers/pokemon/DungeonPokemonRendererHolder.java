...
public class DungeonPokemonRendererHolder extends EntityRendererHolder<DungeonPokemon> {
    ...
    public DungeonPokemonRenderer register(DungeonPokemon pokemon) {
        ...
        if (pokemon.type.isAlliedWith(DungeonPokemonType.TEAM_MEMBER)) {
            if (pokemon.player() == null || pokemon.player() == Persistence.player)
                r.sprite.setShadowColor(PokemonSprite.ALLY_SHADOW);
            else
                r.sprite.setShadowColor(PokemonSprite.PLAYER_SHADOW);
        } else
            r.sprite.setShadowColor(PokemonSprite.ENEMY_SHADOW);
        return r;
    }
