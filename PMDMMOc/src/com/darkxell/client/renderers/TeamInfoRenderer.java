...
public final class TeamInfoRenderer {
	...
	private static void drawMember(Pokemon pokemon, int x, int y, int width, int height) {
		...
		if (width == MEMBER_WIDTH) {
			...
			if (pokemon.species().type2 != null)
				TextRenderer.render(g, pokemon.species().type2.symbol(), x + MEMBER_WIDTH - 2 - symbol, y);
			TextRenderer.render(g, pokemon.species().type1.symbol(),
					x + MEMBER_WIDTH - 2 - symbol - (pokemon.species().type2 == null ? 0 : symbol + 1), y);
		} else {
			Message m = pokemon.species().type1.getName().addSuffix(pokemon.species().type2 == null
					? new Message("", false) : pokemon.species().type2.getName().addPrefix(" "));
			TextRenderer.render(g, m, x + MEMBER_WIDTH * 2 - 5 - TextRenderer.width(m), y);
			m = new Message("team.hp")
					.addReplacement("<max>",
							TextRenderer.alignNumber(pokemon.getBaseStats().getHealth(), 3))
					.addReplacement("<current>", TextRenderer.alignNumber(pokemon.getDungeonPokemon() == null
							? pokemon.getBaseStats().getHealth() : pokemon.getDungeonPokemon().getHp(), 3));
	...
	public static void render(Graphics2D parentGraphics, int canvasWidth, int canvasHeight) {
		...
		if (p == null || p.getTeamLeader() == null) {
			TextRenderer.render(g, loading, canvasWidth / 2 - TextRenderer.width(loading) / 2,
					canvasHeight / 2 - TextRenderer.height() / 2);
			...
			if (team.size() == 1)
				drawMember(team.get(0), 0, y, MEMBER_WIDTH * 2, MEMBER_HEIGHT * 2);
			else
				g.drawLine(0, y + MEMBER_HEIGHT, width, y + MEMBER_HEIGHT);
			...
			if (team.size() == 2 || team.size() == 3)
				drawMember(team.get(0), 0, y, MEMBER_WIDTH * 2, MEMBER_HEIGHT);
			if (team.size() == 2)
				drawMember(team.get(1), 0, y + MEMBER_HEIGHT, MEMBER_WIDTH * 2, MEMBER_HEIGHT);
				...
				drawMember(team.get(team.size() == 3 ? 2 : 3), MEMBER_WIDTH, y + MEMBER_HEIGHT, MEMBER_WIDTH,
						MEMBER_HEIGHT);
			}
		}
