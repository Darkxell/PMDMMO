...
public class RainAnimation extends AbstractAnimation {
    ...
    private void createDrop() {
        ...
        if (this.dropDuration == -1)
            this.dropDuration = a.duration();
    }
    ...
    public void update() {
        ...
        for (SpritesetAnimation animation : this.drops.keySet())
            if (this.drops.get(animation)) {
                ...
                animation.update();
                if (animation.index() == 0 || (this.spritesetID == 103 && animation.index() == 1))
                    animation.setXY(animation.getX() + 1, animation.getY() + 3);
            }
