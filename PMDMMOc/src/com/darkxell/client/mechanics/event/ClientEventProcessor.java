...
public final class ClientEventProcessor extends CommonEventProcessor {
	...
	private void doClientProcess(Event event) {
		...
		if (event instanceof RecruitRequestEvent)
			this.processRecruitRequestEvent((RecruitRequestEvent) event);
		if (event instanceof RecruitAttemptEvent)
			this.processRecruitAttemptEvent((RecruitAttemptEvent) event);
		if (event instanceof RecruitedPokemonEvent)
			this.processRecruitedEvent((RecruitedPokemonEvent) event);
		...
		if (event.mission.owner == Persistence.player) {
			DialogEndListener listener = dialog -> {
				...
				if (((ConfirmDialogScreen) dialog.getScreen(1)).hasConfirmed())
					processEvent(new DungeonExitEvent(Persistence.floor, event, Persistence.player));
				else
					processPending();
			};
	...
	private void processRecruitAttemptEvent(RecruitAttemptEvent event) {
		if (event.eventSource instanceof FaintedPokemonEvent) {
			if (event.hasSucceeded())
				Persistence.dungeonState.pokemonRenderer.getRenderer(event.target).setAlpha(1);
			else
				Persistence.dungeonState.pokemonRenderer.unregister(event.target);
		}
	}
	...
	private void processRecruitRequestEvent(RecruitRequestEvent event) {
		this.setState(State.ANIMATING);
	...
	/**
	 * @return <code>true</code> If having the input event and another
	 *         <i>shouldDelay</i> event should delay the game for a few ticks, to
