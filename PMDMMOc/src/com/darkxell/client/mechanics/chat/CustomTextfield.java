...
public class CustomTextfield {
    ...
    /**
     * Sets this textfield as the selected one. This simply forces the display of the cursor so you don't have two
     * textfields with blinking cursors on a state aty the same time.
     */
    ...
    /**
     * Should be called by a state that uses this textfield upon recieving a KeyPressed event.
     */
    ...
    /**
     * List of all the untypable characters. Trying to type one of them will result in the event being ignored.
     */
    private static char[] untypables = new char[] { '\b', '\t', '\n', '\f', '\r', (char) 0x7f };

    /**
     * Should be called by a state that uses this textfield upon recieving a KeyTyped event.
     */
    public void onKeyTyped(KeyEvent e) {
        if (e.isControlDown())
            return;
        for (char untypable : untypables)
            if (e.getKeyChar() == untypable)
                return;
        if (!unselected)
    ...
    /**
     * Will obfuscate the parsed string if this textfield is in obfuscated mode and rteturn it.
     */
