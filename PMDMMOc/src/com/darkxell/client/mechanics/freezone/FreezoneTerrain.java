...
public class FreezoneTerrain {
	...
	public FreezoneTerrain(String tileset, int width, int height) {
		this.width = width;
		this.height = height;
		this.initTiles();

		AbstractFreezoneTileset ts = this.getTileset(tileset);
		for (int x = 0; x < width; ++x)
			for (int y = 0; y < height; ++y) {
				this.get(x, y).sprite = ts.get(x, y);
			}
		this.defaultTile.sprite = ts.get(0);
	}
	 ...
	 *
	 * @param el {@code <tile />} element, or any element with an {@code x} or
	 *           {@code y} attribute as integers.
	 */
	...
	/**
	 * Get tile based on tile offset. See {@link #get(int, int)} for the conversion
	 * direction.
	 *
	...
	/**
	 * Shortcut for double access to map - simply force-converts double to int, i.e.
	...
	/**
	 * Get total size of map. Should be equal to {@link #getWidth()} times
	 * {@link #getHeight()}.
	 */
	public int size() {
