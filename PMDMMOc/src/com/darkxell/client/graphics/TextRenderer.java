...
public class TextRenderer {
    ...
    public static enum PMDChar {
        ...
        /**
         * @return <code>true</code> if this is an actual displayable character. Chars that may return false are for
         *         example color marks, tabs, etc.
         */
    ...
    public static void render(Graphics2D g, List<PMDChar> text, int x, int y) {
        ...
        for (PMDChar c : text)
            if (!c.isChar()) {
                if (c == PMDChar.colorReset)
                    setColor(null);
                if (c == PMDChar.colorBlue)
                    setColor(Palette.FONT_BLUE);
                if (c == PMDChar.colorGreen)
                    setColor(Palette.FONT_GREEN);
                if (c == PMDChar.colorRed)
                    setColor(Palette.FONT_RED);
    ...
    public static ArrayList<String> splitLines(String text, int boxwidth) {
        ...
        for (String part : parts) {
            ...
            for (String word : words)
                if (!word.startsWith(PMDChar.new_line.value)
                        && (currentlength == 0 || currentlength + width(word) < boxwidth)) {
                    textlines.set(iterator, textlines.get(iterator) + word + " ");
                    currentlength += width(word + " ");
                } else {
                    textlines.add(word + " ");
                    ++iterator;
                    currentlength = width(word + " ");
