...
public class DungeonFloorMap extends AbstractDisplayMap {
	...
	/**
	 * True if the default location needs has been set. Reset to false for each new
	 * floor.
	 */
	private boolean defaultLocationSet = false;
	private Floor floor;
	/**
	 * True if the map should follow the leader. Set to false whenever the player
				...
				for (int y = yStart; y < this.floor.getHeight() && y <= yStart + height / TILE_SIZE; ++y) {
					...
					if (tile == null)
						...
						if (CameraVisibility.check(Visibility::hasSeenTile).test(tile)) {
							...
							else if (CameraVisibility.check(Visibility::isItemVisible).test(tile))
