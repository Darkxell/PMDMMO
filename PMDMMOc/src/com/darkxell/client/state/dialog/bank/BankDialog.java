...
public class BankDialog extends ComplexDialog {
    ...
    public BankDialog(AbstractState background) {
        ...
        this.options = new Message[] { new Message("dialog.bank.deposit"), new Message("dialog.bank.withdraw"),
                new Message("dialog.bank.exit") };
    }
    ...
    private DialogState actionSelection(boolean isFirst) {
        ...
        return this.newDialog(new BankDialogScreen(this.shopkeeper, bag, bank,
                new Message(isFirst ? "dialog.bank.intro" : "dialog.bank.intro2")
                        .addReplacement("<bag>", Long.toString(bag)).addReplacement("<bank>", Long.toString(bank)),
    ...
    public void onConfirmReceived(long bag, long bank) {
        ...
        Persistence.stateManager.setState(this.newDialog(
                new PokemonDialogScreen(this.shopkeeper, new Message("dialog.bank.confirm")).setID(CONFIRM_DIALOG))
                .setOpaque(true));
    }
