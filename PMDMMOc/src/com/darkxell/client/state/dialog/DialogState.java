...
public class DialogState extends AbstractState {
	...
	public DialogState(AbstractGraphicLayer backgroundState, DialogEndListener listener, DialogScreen... screens) {
		...
		if (this.screens.length != 0 && !this.screens[0].shouldRenderBackground())
			this.backgroundAlpha = this.targetAlpha = 0;
	...
	public Rectangle dialogBoxInside() {
		if (this.dialogBox == null) return new Rectangle(0, 0, 1, 1);
		int marginx = this.dialogBox.width / 20, marginy = this.dialogBox.height / 5;
		return new Rectangle(this.dialogBox.x + marginx, this.dialogBox.y + marginy, this.dialogBox.width - marginx * 2,
	...
	public void nextMessage() {
		if (this.currentScreen().requestNextMessage()) {
			...
			else if (this.backgroundState != null && this.currentScreen == this.screens.length - 1
					&& !this.currentScreen().shouldRenderBackground())
				this.targetAlpha = 1 - this.backgroundAlpha;
