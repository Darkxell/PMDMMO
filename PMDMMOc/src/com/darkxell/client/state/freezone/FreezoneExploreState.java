		...
		if (map != null) {
			...
			if (Persistance.debugdisplaymode) {
				...
				TextRenderer.render(g, "UPS: " + Launcher.getUps() + ", FPS: " + Launcher.getFps(), 1,
						TextRenderer.height());
				TextRenderer.render(g, "Position: " + Persistance.currentplayer.x + " / " + Persistance.currentplayer.y,
			...
			if (Persistance.socketendpoint.connectionStatus() != GameSocketEndpoint.CONNECTED) {
				...
				try {
					...
					JsonObject mess = new JsonObject().add("action", "freezoneposition")
							...
							.add("currentpokemon", Persistance.currentplayer.renderer().sprite().pointer.pokemonID + "")
		...
		for (int i = 0; i < Persistance.currentmap.warpzones.size(); i++)
			if (Persistance.currentmap.warpzones.get(i).hitbox.intersects(
				...
				if (destination != null) {
					...
					Persistance.freezoneCamera.x = Persistance.currentplayer.x = wz.toX == -1 ? destination.defaultX()
							: wz.toX;
					Persistance.freezoneCamera.y = Persistance.currentplayer.y = wz.toY == -1 ? destination.defaultY()
							: wz.toY;
				}
				musicset = false;
				break;
