...
public class FreezoneExploreState extends AbstractFreezoneState {
    ...
    public void initiate(FreezoneMap map, Direction direction, int xPos, int yPos) {
        ...
        Persistence.freezoneCamera.x = Persistence.currentplayer.x = xPos == -1 ? Persistence.currentmap.defaultX()
                : xPos;
        Persistence.freezoneCamera.y = Persistence.currentplayer.y = yPos == -1 ? Persistence.currentmap.defaultY()
                : yPos;
    ...
    public void onKeyPressed(Key key) {
        ...
        } else if (Persistence.currentmap != null)
            Persistence.currentplayer.pressKey(key);
    }

    @Override
    public void onKeyReleased(Key key) {
        if (Persistence.currentmap != null)
            Persistence.currentplayer.releaseKey(key);
    }
    ...
    public void onStart() {
        super.onStart();
        if (Key.UP.isPressed())
            Persistence.currentplayer.pressKey(Key.UP);
        if (Key.RIGHT.isPressed())
            Persistence.currentplayer.pressKey(Key.RIGHT);
        if (Key.DOWN.isPressed())
            Persistence.currentplayer.pressKey(Key.DOWN);
        if (Key.LEFT.isPressed())
            Persistence.currentplayer.pressKey(Key.LEFT);
    }
    ...
    public void update() {
        if (Persistence.currentmap == null)
            Persistence.currentmap = new BaseFreezone();
        super.update();
        ...
        if (this.isMain())
            for (int i = 0; i < Persistence.currentmap.triggerzones.size(); i++)
                if (Persistence.currentmap.triggerzones.get(i).hitbox.intersects(Persistence.currentplayer
                        .getHitboxAt(Persistence.currentplayer.x, Persistence.currentplayer.y))) {
