...
public class MoveInfoState extends InfoState
{
	...
	public MoveInfoState(Move move, AbstractState background, AbstractState parent)
	{
		...
		this.details1Message.addReplacement("<range>", new Message("move.info.range." + this.move.targets));
		this.details2Message = new Message("move.info.details.1");
		this.details2Message.addReplacement("<category>", new Message("move.info.category." + this.move.category));
	...
	public void render(Graphics2D g, int width, int height)
	{
		...
		if (this.details1 == null)
		{
			...
			this.details1 = new TextWindow(new Rectangle(r.x, r.y + r.height + 2, r.width / 2 - 1, 2 * TextWindow.MARGIN_Y + 2
					* (TextRenderer.height() + TextRenderer.lineSpacing())), this.details1Message, false);
			this.details2 = new TextWindow(new Rectangle(r.x + r.width / 2 + 2, r.y + r.height + 2, r.width / 2 - 1, 2 * TextWindow.MARGIN_Y + 2
					* (TextRenderer.height() + TextRenderer.lineSpacing())), this.details2Message, false);
			this.details1.leftTab = this.details1.rightTab = this.details1.leftTab = this.details1.rightTab = false;
		}
