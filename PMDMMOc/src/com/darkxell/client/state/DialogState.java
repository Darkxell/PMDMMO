...
public class DialogState extends AbstractState {
	...
	public void onKeyPressed(short key) {
		if (this.endReached && (key == Keys.KEY_ATTACK || key == Keys.KEY_RUN))
	...
	public void render(Graphics2D g, int width, int height) {
		...
		if (this.currentLines.isEmpty())
			this.currentLines.addAll(TextRenderer.instance.splitLines(this.currentMessage().toString(), inside.width));
		...
		int length = 0;
		int offset = 0;
		if (this.endReached && this.switching != -1)
			offset = (int) (-(1f * this.switching / SWITCH_TIME) * inside.height);
		for (int i = 0; i < this.currentLines.size() && length < this.position; ++i) {
			...
			TextRenderer.instance.render(g, this.currentLines.get(i).substring(0, count), inside.x,
					inside.y + offset + i * (TextRenderer.CHAR_HEIGHT + TextRenderer.LINE_SPACING));
			length += count;
		}
		g.setClip(c);

		if (this.endReached && this.switching == -1 && this.arrowtick > 9)
	...
	private boolean switchAnimation() {
		if (this.currentMessage == this.messages.size() - 1)
			return false;
		return this.elements.get(this.currentMessage).pokemon != null && this.elements.get(this.currentMessage).pokemon
				.equals(this.elements.get(this.currentMessage + 1).pokemon);
	}
	...
	public void update() {
		if (!this.endReached && !this.currentLines.isEmpty()) {
			...
			if (this.endReached && Keys.isPressed(Keys.KEY_RUN))
				this.requestNextMessage();
		} else if (this.endReached && this.switching >= 0) {
			++this.switching;
			if (this.switching >= SWITCH_TIME)
				this.nextMessage();
		}
