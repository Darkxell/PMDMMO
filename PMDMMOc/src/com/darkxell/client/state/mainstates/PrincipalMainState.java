...
public class PrincipalMainState extends StateManager {
    ...
    public void onKeyPressed(KeyEvent e, Key key) {
        ...
        if (e.getKeyCode() == KeyEvent.VK_ENTER && isChatFocused)
            Persistence.chatbox.send();
        if (isChatFocused)
            Persistence.chatbox.textfield.onKeyPressed(e);
        if (isChatFocused && e.getKeyCode() == KeyEvent.VK_UP)
            Persistence.chatbox.up();
        if (e.getKeyCode() == KeyEvent.VK_F12 && e.isShiftDown())
            Persistence.debugdisplaymode = !Persistence.debugdisplaymode;
        if (e.getKeyCode() == KeyEvent.VK_F11 && e.isShiftDown())
            Persistence.debugwiresharkmode = !Persistence.debugwiresharkmode;
        if (e.getKeyCode() == KeyEvent.VK_F10 && e.isShiftDown())
            Persistence.debugcommunicationmode = !Persistence.debugcommunicationmode;
    }
    ...
    public void onKeyTyped(KeyEvent e) {
        if (isChatFocused)
            Persistence.chatbox.textfield.onKeyTyped(e);
        if (isGameFocused)
    ...
    private Point inGameLocation(int x, int y) {
        if (this.internalBuffer == null || this.gamewidth == 0 || this.gameheight == 0)
            return null;
    ...
    public void render(Graphics2D g, int width, int height) {
        ...
        if (Persistence.displaymap == null) {
            ...
            Persistence.displaymap.render(g, mapsize, mapsize);
        ...
        if (size.first != 0 && size.second != 0)
            MainUiUtility.drawBoxOutline(g, gamex, mapy, size.first + 1, size.second + 1);
    }
