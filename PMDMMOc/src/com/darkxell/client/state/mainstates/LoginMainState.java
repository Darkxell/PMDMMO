...
public class LoginMainState extends StateManager
{
	...
	public void onMouseClick(int x, int y)
	{
		...
		{
			if (Persistance.socketendpoint.connectionStatus() == GameSocketEndpoint.CONNECTED) Persistance.stateManager = new AccountCreationState();
		} else if (button_login.isInside(new Position(mouseX - offsetx, mouseY - offsety)))
		{
			if (Persistance.socketendpoint.connectionStatus() == GameSocketEndpoint.CONNECTED) launchOnlineSend();
	...
	public void render(Graphics2D g, int width, int height)
	{
		...
		g.setColor((button_login.isInside(new Position(relativemousex, relativemousey))) ? selectedbuttoncolor : buttoncolor);
		...
		g.setColor((button_createaccount.isInside(new Position(relativemousex, relativemousey))) ? selectedbuttoncolor : buttoncolor);
		if (Persistance.socketendpoint.connectionStatus() != GameSocketEndpoint.CONNECTED) g.setColor(new Color(120, 120, 180));
		g.fillRect((int) button_createaccount.x, (int) button_createaccount.y, (int) button_createaccount.width, (int) button_createaccount.height);
		g.setColor(Color.BLACK);
		g.drawString("CREATE ACCOUNT", (int) button_createaccount.x + 15, (int) button_createaccount.y + 20);
		g.setColor((button_offline.isInside(new Position(relativemousex, relativemousey))) ? selectedbuttoncolor : buttoncolor);
		g.fillRect((int) button_offline.x, (int) button_offline.y, (int) button_offline.width, (int) button_offline.height);
		g.setColor(Color.BLACK);
	...
	public void update()
	{
		...
		if (firstupdate)
		{
			...
			this.password.setSelection(false);
			this.sendSaltReset();
			if (this.login.getContent().equals("")) this.login.insertString(ClientSettings.getSetting(ClientSettings.LOGIN));
		}
	}
	...
	private void sendLogin()
	{
		...
		try
		{
			localhash = Encryption.clientHash(this.password.getContent(), this.login.getContent(), Encryption.HASHSALTTYPE_CLIENT);
			String serverhash = Encryption.clientHash(localhash, this.login.getContent(), Encryption.HASHSALTTYPE_SERVER);
			String loginhash = Encryption.clientHash(serverhash, localsalt, Encryption.HASHSALTTYPE_LOGIN);
			JsonObject message = new JsonObject().add("action", "login").add("name", this.login.getContent()).add("passhash", loginhash);
