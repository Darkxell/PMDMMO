...
public class PokemonTravelState extends DungeonSubState {
	...
	public void onStart() {
		...
		for (PokemonTravel travel : this.travels) {
			...
			if (sprite.getState() != PokemonSprite.STATE_MOVE)
				sprite.setState(PokemonSprite.STATE_MOVE);
	...
	public void render(Graphics2D g, int width, int height) {
		for (int i = 0; i < this.travels.length; ++i)
			DungeonPokemonRenderer.instance.draw(g, this.travels[i].pokemon, this.animations[i].current().getX(),
					this.animations[i].current().getY());
	}

	private boolean shouldStopRunning(DungeonPokemon runner, PokemonTravel travel) {
		if (!runner.tryMoveTo(runner.facing()))
			return true;
	...
	public void update() {
		...
		if (this.tick >= this.duration) {
			...
			for (PokemonTravel travel : this.travels) {
				...
				if (travel.destination.type() == TileType.STAIR)
					stairLand = travel.pokemon == Persistance.player.getDungeonPokemon();
			}
			...
			if (this.running)
				shouldStop = this.shouldStopRunning(travel.pokemon, travel) || ClientEventProcessor.hasPendingEvents()
						|| Persistance.dungeon.getNextActor() != null;
			else
				shouldStop = direction == -1 || ClientEventProcessor.hasPendingEvents()
						|| Persistance.dungeon.getNextActor() != null;
			...
			if (shouldStop)
				...
				if (ClientEventProcessor.hasPendingEvents())
					this.stopTravel();
				else if (this.running)
					ClientEventProcessor.actorTravels(travel.pokemon.facing(), true);
			}
		}
	}
}
