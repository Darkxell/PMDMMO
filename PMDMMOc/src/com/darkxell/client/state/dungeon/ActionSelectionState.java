...
public class ActionSelectionState extends DungeonSubState {
    ...
    public Direction checkMovement() {
        ...
        if (Keys.directionPressed(canRun, Key.UP, Key.RIGHT))
            direction = Direction.NORTHEAST;
        else if (Keys.directionPressed(canRun, Key.DOWN, Key.RIGHT))
            direction = Direction.SOUTHEAST;
        else if (Keys.directionPressed(canRun, Key.DOWN, Key.LEFT))
            direction = Direction.SOUTHWEST;
        else if (Keys.directionPressed(canRun, Key.UP, Key.LEFT))
            direction = Direction.NORTHWEST;

        else if (!this.parent.diagonal)
            if (Keys.directionPressed(canRun, Key.UP))
                direction = Direction.NORTH;
            else if (Keys.directionPressed(canRun, Key.DOWN))
                direction = Direction.SOUTH;
            else if (Keys.directionPressed(canRun, Key.LEFT))
                direction = Direction.WEST;
            else if (Keys.directionPressed(canRun, Key.RIGHT))
    ...
    public void onKeyPressed(Key key) {
        ...
        if (key == Key.MENU)
            ...
            if (i != null)
                Persistence.stateManager.setState(i);
        } else if (key == Key.PARTY)
            Persistence.stateManager.setState(s.createPartyState());
        else if (key == Key.RUN && Key.ROTATE.isPressed())
            this.parent.setSubstate(new DungeonFullLoggerState(this.parent));
        else if (key == Key.ROTATE) {
            Direction d = leader.facing();
            do {
                d = d.rotateClockwise();
                if (leader.tile().adjacentTile(d).getPokemon() != null
                        && !leader.player().isAlly(leader.tile().adjacentTile(d).getPokemon()))
                    break;
        ...
        if (Persistence.player.getDungeonLeader().canAttack(Persistence.floor) && key == Key.MOVE_1 || key == Key.MOVE_2
            ...
            if (key != Key.ATTACK && Persistence.player.getDungeonLeader().isStruggling()) {
                Persistence.eventProcessor().processEvent(
                        new MoveSelectionEvent(Persistence.floor, eventSource, new LearnedMove(MoveRegistry.STRUGGLE.id), leader)
                                .setPAE());
                done = true;
            }
            ...
            if (!done) {
                ...
                if (move != null)
                    if (move.pp() == 0)
                        ...
                        this.parent.logger.showMessage(
                ...
                if (key == Key.ATTACK && (!Key.RUN.isPressed() || Persistence.player.getDungeonLeader().isFamished())) {
                    DungeonPokemon facing = Persistence.player.getDungeonLeader().tile()
                            .adjacentTile(Persistence.player.getDungeonLeader().facing()).getPokemon();
                    ...
                    if (Persistence.player.getDungeonLeader().canAttack(Persistence.floor))
                        Persistence.eventProcessor()
                                .processEvent(new MoveSelectionEvent(Persistence.floor,
                                        eventSource, new LearnedMove(MoveRegistry.ATTACK.id), Persistence.player.getDungeonLeader())
                                                .setPAE());
                }
            }
        }
        ...
        if (this.moveLocations != null)
            for (byte i = 0; i < this.moveLocations.length; ++i)
                if (this.moveLocations[i] != null && this.moveLocations[i].contains(x, y)) {
                    Persistence.eventProcessor()
                            .processEvent(new MoveSelectionEvent(Persistence.floor,
                                    eventSource, Persistence.player.getTeamLeader().move(i), Persistence.player.getDungeonLeader())
                                            .setPAE());
                    break;
                }
    }
    ...
    public void prerender(Graphics2D g, int width, int height) {
        ...
        if (this.delay >= 3 && this.parent.rotating)
            if (!this.parent.diagonal)
                this.drawArrow(g, width, height, Persistence.player.getDungeonLeader().facing());
    }
    ...
    public void render(Graphics2D g, int width, int height) {
        ...
        if (this.movesWindow == null)
            this.movesWindow = new MenuWindow(
                    new Rectangle((width - w) / 2, height - MOVES_WINDOW_HEIGHT - 5, w, MOVES_WINDOW_HEIGHT));
            ...
            if (this.delay >= 3 && this.movesWindow != null && !this.parent.logger.isVisible()) {
                ...
                for (int i = 0; i < 4; ++i)
                    if (leader.move(i) != null) {
                        ...
                        if (leader.move(i).move() != null)
                            name = leader.move(i).move().name().toString();
                        this.moveLocations[i] = new Rectangle(this.movesWindow.inside().x + 5 + i * mw - 1, y - 1,
                                TextRenderer.width(name) + 2, TextRenderer.height() + 2);
                        if (i == this.hoveredMove) {
                            g.setColor(Palette.MENU_HOVERED);
                            g.fillRect(this.moveLocations[i].x, this.moveLocations[i].y, this.moveLocations[i].width,
                                    this.moveLocations[i].height);
                        }
                        TextRenderer.render(g, name, this.movesWindow.inside().x + 5 + i * mw, y);
                    } else
                        this.moveLocations[i] = null;
            }
    }
    ...
    public void update() {
        ...
        if (this.rotationCounter > ROTATION_COUNTER_MAX)
