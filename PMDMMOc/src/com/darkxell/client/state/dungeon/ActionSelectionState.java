...
public class ActionSelectionState extends DungeonSubState {
    ...
    public Direction checkMovement() {
        ...
        else if (!this.parent.diagonal) if (Keys.directionPressed(canRun, Key.UP)) direction = Direction.NORTH;
        ...
        if (direction != null) {
            ...
            if (direction != leader.facing()) Persistence.eventProcessor().processEvent(
    ...
    public void onKeyPressed(Key key) {
        ...
        if (key == Key.MENU) Persistence.stateManager.setState(s);
            ...
            if (d != leader.facing()) Persistence.eventProcessor().processEvent(
                    new PokemonRotateEvent(Persistence.floor, DungeonEventSource.PLAYER_ACTION, leader, d).setPAE());
        }
        ...
        if (Persistence.player.getDungeonLeader().canAttack(Persistence.floor) && key == Key.MOVE_1 || key == Key.MOVE_2
            ...
            if (!done) {
                ...
                if (move != null) if (move.pp() == 0) this.parent.logger
                    ...
                    if (facing != null && facing.type == DungeonPokemonType.RESCUEABLE) {
                        ...
                        if (m != null && m.owner == Persistence.player) {
                            Persistence.eventProcessor().processEvent(
                                    new PokemonRescuedEvent(Persistence.floor, eventSource, facing, Persistence.player)
                                            .setPAE());
                            return;
                        }
                    }
    ...
    public void render(Graphics2D g, int width, int height) {
        int w = width - 40;
        if (this.movesWindow == null) this.movesWindow = new MenuWindow(
                new Rectangle((width - w) / 2, height - MOVES_WINDOW_HEIGHT - 5, w, MOVES_WINDOW_HEIGHT));

        if (this.isMain()) if (this.delay >= 3 && this.movesWindow != null && !this.parent.logger.isVisible()) {
