...
public class Keys implements KeyListener {
	public enum Key {
		...
		public static Key getKeyFromID(int keyID) {
			for (Key key : Key.values()) {
				if (key.keyValue() == keyID) { return key; }
			}
		...
		public int keyValue() {
			if (this.setting == null) { return this.lastAssigned; }
	 ...
	 * <p/>
	 * If the {@link Key#RUN RUN} key is pressed, check that the key was not already pressed - this will only return true once per run cycle (i.e. press run, release run).
	 *
