	...
	public CustomListCell(ListCellParent<T> parent, boolean orderable)
	{
		...
		if (orderable)
		{
			...
			this.setOnDragEntered(event -> {
				if (event.getGestureSource() != this && event.getDragboard().hasString()) this.setOpacity(0.3);
			});

			this.setOnDragExited(event -> {
				if (event.getGestureSource() != this && event.getDragboard().hasString()) this.setOpacity(1);
			});
			...
			this.setOnDragDropped(event -> {
				...
				if (db.hasString())
				{
					...
					if (draggedIdx < thisIdx - 1) --thisIdx;
