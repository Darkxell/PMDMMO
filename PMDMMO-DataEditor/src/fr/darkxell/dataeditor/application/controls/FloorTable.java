...
public class FloorTable
{
	...
	public static class FloorTableManager<T, D>
	{
		...
		public void onFloorEdited(CellEditEvent<FloorTableItem<T, D>, String> event)
		{
			...
			if (!value.matches("\\d+")) FXUtils.showAlert("Invalid floor: " + value);
			else if (value.equals("0")) FXUtils.showAlert("Bruh, above 0 please. What are you doing");
			else selected.floor = Integer.parseInt(value);
			this.sort();
		}
	...
	private static <T, D> ContextMenu createContextMenu(TableView<FloorTableItem<T, D>> table, String objectName, FloorTableItem<T, D> defaultItem,
			FloorTableManager<T, D> manager)
	{
