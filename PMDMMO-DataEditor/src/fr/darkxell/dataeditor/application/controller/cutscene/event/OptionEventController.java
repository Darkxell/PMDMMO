...
public class OptionEventController extends EventController {
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        down.setOnAction(e -> {
            int i = this.optionsTable.getSelectionModel().getSelectedIndex();
            DialogOptionTableItem item = this.optionsTable.getItems().remove(i);
            if (i == this.optionsTable.getItems().size() - 2)
                this.optionsTable.getItems().add(item);
            else
                this.optionsTable.getItems().add(i + 1, item);
        });
        ...
        menu.setOnShowing(e -> {
            ...
            up.setDisable(this.optionsTable.getSelectionModel().isEmpty()
                    || this.optionsTable.getSelectionModel().getSelectedIndex() == 0);
            down.setDisable(this.optionsTable.getSelectionModel().isEmpty() || this.optionsTable.getSelectionModel()
                    .getSelectedIndex() == this.optionsTable.getItems().size() - 1);
        });
    ...
    private void onOptionEdited(CellEditEvent<DialogOptionTableItem, String> event) {
        if (event.getNewValue().equals(""))
            FXUtils.showAlert("Bruh please put it something");
        else
            event.getRowValue().message = event.getNewValue();
    ...
    private void onTranslateEdited(CellEditEvent<DialogOptionTableItem, String> event) {
        ...
        if (isTrue)
            ...
            FXUtils.showAlert("Invalid boolean: " + newValue);

        if (isTrue || isFalse)
            event.getRowValue().translate = value;
