...
public class MovesTabController implements Initializable, ListCellParent<MoveListItem>
{
	...
	public void initialize(URL location, ResourceBundle resources)
	{
		...
		for (PokemonType type : PokemonType.values())
			categories.add(new TreeItem<CustomTreeItem>(new TreeCategory(type.getName().toString().replaceAll("<.*?>", "") + "-type Moves")));
		...
		/* this.movesList.setCellFactory(param -> { return new CustomListCell<>(AnimationsTabController.instance, "Animation").setCanOrder(false).setCanCreate(false).setCanDelete(false) .setCanRename(false); }); */
		this.movesTreeView.setOnMouseClicked(new EventHandler<MouseEvent>() {
			@Override
		...
		this.reloadList();/* CustomList.setup(this, this.movesList, "Move", true, false, true, true, false); this.movesList.getMoves().addAll(MoveRegistry.list()); this.movesList.getMoves().sort(Comparator.naturalOrder()); */
	...
	public void onDeleteMove()
	{
		...
		if (selected.getValue() instanceof MoveListItem) this.onDelete((MoveListItem) selected.getValue());
	}
	...
	private void reloadList()
	{
		...
		for (Move i : Registries.moves().toList())
		{
			...
			else if (i.id >= 2500) tree = this.categories[this.categories.length - 2];
			else if (i.type == PokemonType.Unknown) tree = this.categories[this.categories.length - 3];
			else tree = this.categories[i.type.id];
