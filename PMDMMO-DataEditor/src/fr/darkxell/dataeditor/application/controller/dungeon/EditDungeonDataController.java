...
public class EditDungeonDataController implements Initializable
{
			...
			throws DungeonCreationException
	{
		...
		if (this.linkedCheckbox.isSelected())
		{
			...
			if (linked == null) throw new DungeonCreationException("No Linked Dungeon was chosen");
			else linkedTo = linked.id;
		}

		return new Dungeon(id, floorCount, this.directionCombobox.getValue(), this.recruitsCheckbox.isSelected(), timeLimit, stickyChance, linkedTo, pokemon,
				items, shopItems, buriedItems, traps, floorData, weather, mapx, mapy);
	}
