...
public class SessionsInfoHolder {
    ...
    /**predicate that returns true if there is data concerning the parsed session available.*/
    public static boolean infoExists(String sessionID){
        return infos.containsKey(sessionID);
    }
    
    /**Creates default values for a session in memory. Does nothing it the session already has informations.*/
