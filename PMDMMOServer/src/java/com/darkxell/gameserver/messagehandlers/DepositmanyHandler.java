 ...
 * @author Darkxell
 */
public class DepositmanyHandler extends MessageHandler {

    public DepositmanyHandler(GameServer endpoint) {
        super(endpoint);
    }
    ...
    public void handleMessage(JsonObject json, Session from, GameSessionHandler sessionshandler) {
        ...
        if (ids.length + p_storage.content.size() > p_storage.maxsize) {
        ...
        for (int i = 0; i < ids.length; i++) {
            ...
            if (inventoryid == p_toolbox.id) {
                endpoint.getInventoryContains_DAO().delete(ids[i], p_toolbox.id);
                endpoint.getInventoryContains_DAO().create(ids[i], p_storage.id);
            } else {
                error = true;
            }
        }
