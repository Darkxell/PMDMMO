...
/**
 * Represents the behavior at the Dungeon-level while in an exploration (Floor
 * management, Players, AI, seeds, turns & events memory...).
 */
public class DungeonExploration {
	...
	public final int id;
	/**
	 * True if this Dungeon is currently generating a floor. Used for Actor
	 * registering.
	 */
	...
	/**
	 * Adds the input Player to the list of Players currently exploring this
	 * Dungeon.
	 */
	...
	/**
	 * Creates the next Floor to explore, places Players in it, registers Actors,
	 * AIs, and IDs.
	 */
	...
	/**
	 * @return The Pokemon taking its turn. null if there is no actor left, thus the
	 *         turn is over.
	 */
	...
	/*
	 * public void insertActor(DungeonPokemon pokemon, int index) { if
	 * (this.actorMap.containsKey(pokemon)) return; this.actorMap.put(pokemon, new
	 * Actor(pokemon)); this.actors.add(index, this.actorMap.get(pokemon)); }
	 */
	...
	/**
	 * Starts the current exploration. Creates the first floor and starts the Event
	 * Processor.<br>
	 * /!\ Make sure the Event Processor has been initialized if you want a custom
	 * one. /!\
	 *
	...
	public void registerActor(DungeonPokemon pokemon) {
		...
		if (pokemon.player() == null)
			...
			for (index = 0; index < this.actors.size(); ++index) {
				if (this.actors.get(index).pokemon.player() == pokemon.player())
					playerFound = true;
				else if (playerFound)
					break;
			}
	...
	/**
	 * Removes the input Player from this Dungeon. Called when that Player exits the
	 * Dungeon by winning, escaping or losing.
	 *
