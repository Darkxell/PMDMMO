        ...
        if (event instanceof MoveUseEvent) {
            ...
            if (e.target == concerned && !e.hasFlag("redirected")) {
                ...
                MoveUseEvent redirected = new MoveUseEvent(floor, e.eventSource,
                        new MoveUse(floor, e.usedMove.move, concerned, concerned.facing(), e.eventSource), e.usedMove.user);
                redirected.addFlag("redirected");
                resultingEvents
                        .add(new MessageEvent(floor, event, new Message("status.trigger.43").addReplacement("<pokemon>", concerned.toString())));
