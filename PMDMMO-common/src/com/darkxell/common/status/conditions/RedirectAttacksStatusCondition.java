...
public class RedirectAttacksStatusCondition extends StatusCondition
{
	...
	public RedirectAttacksStatusCondition(int id, boolean isAilment, int durationMin, int durationMax, double redirectedMultiplier)
	{
	...
	public void onPreEvent(Floor floor, DungeonEvent event, DungeonPokemon concerned, ArrayList<DungeonEvent> resultingEvents)
	{
		...
		if (event instanceof MoveUseEvent)
		{
			...
			{
				event.consume();
				MoveUseEvent redirected = new MoveUseEvent(floor, new MoveUse(floor, e.usedMove.move, concerned, concerned.facing()), e.usedMove.user);
				redirected.addFlag("redirected");
				resultingEvents.add(new MessageEvent(floor, new Message("status.trigger.43").addReplacement("<pokemon>", concerned.toString())));
