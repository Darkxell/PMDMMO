...
public class PropertyModificator
{
	...
	public int applyCriticalRateModifications(int critical, MoveUse move, DungeonPokemon target, Floor floor)
	{
		for (AffectsPokemon affecter : this.affecters)
			critical = affecter.applyCriticalRateModifications(critical, move, target, this.users.contains(affecter), floor);
		return critical;
	}
	...
	{
		for (AffectsPokemon affecter : this.affecters)
			stage = affecter.applyStatStageModifications(stat, stage, move, target, this.users.contains(affecter), floor);
		return stage;
	}
	...
	public double damageMultiplier(MoveUse move, DungeonPokemon target, Floor floor)
	{
		double multiplier = 1;
		for (AffectsPokemon affecter : this.affecters)
			multiplier *= affecter.damageMultiplier(move, target, this.users.contains(affecter), floor);
		return multiplier;
	}
