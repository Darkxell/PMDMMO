...
public class PokemonSpecies implements Registrable<PokemonSpecies> {
    ...
    public PokemonSpecies(Element xml) {
        ...
        if (xml.getChild("experience", xml.getNamespace()) != null) {
            ...
            this.experiencePerLevel = new int[0];
    ...
    public PokemonSpecies(int id, int formID, PokemonType type1, PokemonType type2, int baseXP,
            ArrayList<BaseStats> baseStats, float height, float weight, Mobility mobility, ArrayList<Integer> abilities,
            int[] experiencePerLevel, HashMap<Integer, ArrayList<Integer>> learnset, ArrayList<Integer> tms,
    ...
    private PokemonSpecies createForm(Element xml) {
        ...
        PokemonType type1 = xml.getAttribute("type1") == null ? this.type1
        ...
        Mobility mobility = xml.getAttribute("mobility") == null ? this.mobility
                : Mobility.valueOf(xml.getAttributeValue("mobility"));
        ...
        if (xml.getChild("experience", xml.getNamespace()) == null)
            experiencePerLevel = this.experiencePerLevel.clone();
        else {
            String[] lvls = xml.getChildText("experience", xml.getNamespace()).split(",");
        ...
        if (xml.getChild("evolves", xml.getNamespace()) == null)
            evolutions = (ArrayList<Evolution>) this.evolutions.clone();
        else
            for (Element e : xml.getChild("evolves", xml.getNamespace()).getChildren())
                evolutions.add(new Evolution(e));

        if (xml.getChild("tms", xml.getNamespace()) == null)
            tms = (ArrayList<Integer>) this.tms.clone();
        else
            for (Element tm : xml.getChild("tms", xml.getNamespace()).getChildren())
                tms.add(Integer.parseInt(tm.getAttributeValue("id")));

        if (xml.getChild("learnset", xml.getNamespace()) == null)
            learnset = (HashMap<Integer, ArrayList<Integer>>) this.learnset.clone();
        else
            for (Element level : xml.getChild("learnset", xml.getNamespace()).getChildren())
        ...
        return new PokemonSpecies(id, formID, type1, type2, baseXP, baseStats, height, weight, mobility, abilities,
                experiencePerLevel, learnset, tms, evolutions, forms, friendArea);
    }

    private Mobility defaultMobility() {
        if (this.isType(PokemonType.Ghost))
            return Mobility.Ghost;
        if (this.isType(PokemonType.Flying) || this.abilities.contains(Ability.LEVITATE.id))
    ...
    public Element toXML() {
        ...
        if (this.type2 != null)
            root.setAttribute("type2", this.type2.name());
        ...
        if (this.mobility != this.defaultMobility())
            root.setAttribute("mobility", this.mobility.name());
        ...
        if (this.tms.size() != 0)
            root.addContent(XMLUtils.toXML("tms", this.tms));
    ...
    private void toXML(Element root, PokemonSpecies form) {
        ...
        if (this.type1 != form.type1)
            e.setAttribute("type1", form.type1.name());
        if (this.type2 != form.type2)
            e.setAttribute("type2", form.type2 == null ? "null" : form.type2.name());
        if (this.baseXP != form.baseXP)
            e.setAttribute("base-xp", Integer.toString(form.baseXP));
        if (this.height != form.height)
            e.setAttribute("height", Float.toString(form.height));
        if (this.weight != form.weight)
            e.setAttribute("weight", Float.toString(form.weight));
        if (this.mobility != form.mobility)
            e.setAttribute("mobility", form.mobility.name());
        ...
        if (!this.abilities.equals(form.abilities))
            e.addContent(XMLUtils.toXML("abilities", form.abilities));

        if (!this.tms.equals(form.tms))
            e.addContent(XMLUtils.toXML("tms", form.tms));
