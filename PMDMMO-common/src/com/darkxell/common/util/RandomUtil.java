...
public final class RandomUtil {
     ...
     */
    public static <T> T random(List<T> candidates, Random random) {
    ...
    public static int randomize(int number, Random random) {
        if (number < 3)
            return nextGaussian(number - 1, 1.5, random);
     ...
     * @param  random  - A Random Number Generator.
     * @return         A random Object from the input List. The greater an Object's Weight is, the more likely it is to
     *                 be chosen.
     */
    public static <T> T weightedRandom(List<T> candidates, List<Integer> weights, Random random) {
        if (candidates.isEmpty()) {
