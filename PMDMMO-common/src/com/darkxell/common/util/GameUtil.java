...
public class GameUtil {
	 ...
	 * </ul>
	 */
	public static final short NORTH = 1, NORTHEAST = 2, EAST = 4, SOUTHEAST = 8, SOUTH = 16, SOUTHWEST = 32, WEST = 64,
			NORTHWEST = 128;

	/** Lists directions. */
	private static short[] directions = new short[] { NORTH, NORTHEAST, EAST, SOUTHEAST, SOUTH, SOUTHWEST, WEST,
			NORTHWEST };

	/**
	 * Removes corner directions that are not connected to both adjacent
	 * directions.
	 */
	public static short clean(short neighbors) {
		if (containsDirection(neighbors, NORTHEAST)
				&& !(containsDirection(neighbors, NORTH) && containsDirection(neighbors, EAST)))
			neighbors -= NORTHEAST;
		if (containsDirection(neighbors, NORTHWEST)
				&& !(containsDirection(neighbors, NORTH) && containsDirection(neighbors, WEST)))
			neighbors -= NORTHWEST;
		if (containsDirection(neighbors, SOUTHEAST)
				&& !(containsDirection(neighbors, SOUTH) && containsDirection(neighbors, EAST)))
			neighbors -= SOUTHEAST;
		if (containsDirection(neighbors, SOUTHWEST)
				&& !(containsDirection(neighbors, SOUTH) && containsDirection(neighbors, WEST)))
			neighbors -= SOUTHWEST;
		return neighbors;
	}
	...
	public static boolean containsDirection(short directionSum, short direction) {
		for (int i = directions.length - 1; i >= 0; --i) {
			if (directions[i] == direction)
				return directionSum >= directions[i];
			if (directionSum >= directions[i])
	...
	/**
	 * @return The coordinates given when moving from the input X,Y coordinates
	...
	/**
	 * @return The next direction after rotating clockwise from the input
	 *         direction.
	 */
