...
public class RandomStatChangeEffect extends MoveEffect
{
			...
			MoveEvents effects)
	{
		...
		if (!missed && floor.random.nextDouble() * 100 < this.probability)
		{
			...
			effects.createEffect(new StatChangedEvent(floor, changed, this.stat(floor), this.stage, usedMove), usedMove, target, floor, missed,
	...
	{
		return new Message(this.descriptionID()).addReplacement("<stage>", String.valueOf((int) Math.abs(this.stage))).addReplacement("<percent>",
