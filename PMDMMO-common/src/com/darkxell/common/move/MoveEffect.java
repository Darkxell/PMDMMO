...
public class MoveEffect implements AffectsPokemon {
    ...
    public Message description(Move move) {
        Message m;
        if (Localization.containsKey("move.info." + this.id)) m = new Message("move.info." + this.id);
    ...
    public DungeonPokemon[] getTargets(Move move, DungeonPokemon user, Floor floor) {
        ...
        if (move.range == MoveRange.Room || move.range == MoveRange.Floor) targets.sort(floor.dungeon::compare);
        ...
        if (moveEvent.target != null)
            if (missed)
                effects.createEffect(
                        new MessageEvent(moveEvent.floor, moveEvent,
                                new Message(moveEvent.target == null ? "move.miss.no_target" : "move.miss")
                                        .addReplacement("<pokemon>",
                                                moveEvent.target == null ? new Message("no one", false)
                                                        : moveEvent.target.getNickname())),
                        moveEvent, missed, true);
            else if (moveEvent.usedMove.move.move().dealsDamage) effects
                    .createEffect(
                            new DamageDealtEvent(moveEvent.floor, moveEvent, moveEvent.target, moveEvent.usedMove,
                                    DamageType.MOVE, calculator.compute(effects.events)),
    ...
    public boolean mainUse(MoveUseEvent moveEvent, ArrayList<Event> events) {
        ...
        if (calculator == null) calculator = new MoveEffectCalculator(moveEvent);
        ...
        if (effectiveness == PokemonType.NO_EFFECT && moveEvent.usedMove.move.move().category != MoveCategory.Status)
            ...
            if (!missed && this != MoveEffects.Basic_attack && moveEvent.target != null) moveEvent.target.receiveMove(
                    moveEvent.usedMove.move.isLinked() ? DungeonPokemon.LINKED_MOVES : DungeonPokemon.MOVES);
            if (!missed && move.dealsDamage) if (effectiveness >= PokemonType.SUPER_EFFECTIVE)
