...
public class MoveEvents {
     ...
     *                     <code>null</code> if no actual target. This may be important in the triggering of certain
     *                     abilities.
     */
    public void createEffect(DungeonEvent effect, MoveUse usedMove, DungeonPokemon target, Floor floor, boolean missed,
            boolean isAdditional, DungeonPokemon directedAt) {
        if (usedMove.user.ability() instanceof AbilityModifyMoveEffect)
            effect = ((AbilityModifyMoveEffect) usedMove.user.ability()).modify(effect, usedMove, target, floor, missed,
                    isAdditional, true, directedAt);

        if (effect != null && target != null && target.ability() instanceof AbilityModifyMoveEffect)
            effect = ((AbilityModifyMoveEffect) target.ability()).modify(effect, usedMove, target, floor, missed,
