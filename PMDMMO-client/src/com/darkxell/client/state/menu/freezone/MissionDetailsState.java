...
public class MissionDetailsState extends AbstractState {
    ...
    public void render(Graphics2D g, int width, int height) {
        ...
        for (int i = 0; i < mission.getRewards().getItems().length && i < 3; i++) {
            TextRenderer.render(g, mission.getRewards().getQuantities()[i] + "x <blue>"
                    + items.find(mission.getRewards().getItems()[i]).name() + "</color>", 40, rewardsoffsety);
            rewardsoffsety += 15;
        }
        // Resets the graphics
        g.translate(-offsetx, -offsety);
    ...
    /**
     * Notifies this state that the mission has been accepted (by the server typically) or not.
     */
