...
public class FriendAreaSelectionMapState extends AbstractState {
    ...
    public void render(Graphics2D g, int width, int height) {
        ...
        // DRAWS THE MAP
        g.drawImage(MapSprites.localMap.image(), 0, 0, null);
        for (LocalMapLocation info : this.points)
            if (info.showsonfriendsmap) {
                boolean text = isnearpoint(info);
                g.drawImage(
                        info == LocalMapLocation.BASE ? MapSprites.pins.red()
                                : text ? MapSprites.pins.green() : MapSprites.pins.yellow(),
                ...
                if (text) {
                    ...
                    if (leftpart <= 0)
                        leftpart = 0;
                    else if (leftpart + twidth + 8 >= MapSprites.localMap.image().getWidth())
                        leftpart = MapSprites.localMap.image().getWidth() - twidth - 8;
    ...
    public void update() {
        ...
        if (movingright)
            newcursorx += cursorspeed;
        if (newcursorx >= 0 && newcursorx <= MapSprites.localMap.image().getWidth())
            cursorx = newcursorx;
        if (movingdown)
            newcursory += cursorspeed;
        if (movingup)
            newcursory -= cursorspeed;
        if (newcursory >= 0 && newcursory <= MapSprites.localMap.image().getHeight())
            cursory = newcursory;
        // Camera X
        ...
        if (newcamerax < memorywidth / 2) {
            if (camerax < memorywidth / 2)
                camerax += cameraspeed;
        } else if (newcamerax > MapSprites.localMap.image().getWidth() - (memorywidth / 2)) {
            if (camerax > MapSprites.localMap.image().getWidth() - (memorywidth / 2))
                camerax -= cameraspeed;
        } else
            camerax = newcamerax;
        // Camera Y
        ...
        if (newcameray < memoryheight / 2) {
            if (cameray < memoryheight / 2)
                cameray += cameraspeed;
        } else if (newcameray > MapSprites.localMap.image().getHeight() - (memoryheight / 2)) {
            if (cameray > MapSprites.localMap.image().getHeight() - (memoryheight / 2))
                cameray -= cameraspeed;
        } else
            cameray = newcameray;
    }
    ...
    /**
     * Predicate that returns true if the cursor is on the right side of the screen of this state, meaning the UI above
     * should be displayed on the left instead.
     */
    public boolean shouldInvertUI() {
        return cursorx > MapSprites.localMap.image().getWidth() - (this.memorywidth / 3);
    }
