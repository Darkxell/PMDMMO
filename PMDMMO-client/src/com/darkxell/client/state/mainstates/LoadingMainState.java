...
public class LoadingMainState extends StateManager {
    ...
    public void onLoadingFinished() {
        if (ClientSettings.getSetting(ClientSettings.LANGUAGE).equals("null")) {
            if (Language.values().length == 1) {
                ClientSettings.setSetting(ClientSettings.LANGUAGE, Language.values()[0].id);
                Persistence.stateManager = new LoginMainState();
            } else
                Persistence.stateManager = new ChooseLanguageMainState();
        } else
            Persistence.stateManager = new LoginMainState();
    }
