...
public class FreezonePlayer {
    ...
    /**
     * Returns false if the player would collide in a solid tile/tileentity if it were at the wanted location, true
     * otherwise.
     */
    public boolean canBeAt(double x, double y) {
        ...
        if (Persistence.currentmap.getTileAt((int) hbx.x, (int) hbx.y).isSolid())
            return false;
        if (Persistence.currentmap.getTileAt((int) hbx.x, (int) (hbx.y + hbx.height)).isSolid())
            return false;
        if (Persistence.currentmap.getTileAt((int) (hbx.x + hbx.width), (int) hbx.y).isSolid())
            return false;
        if (Persistence.currentmap.getTileAt((int) (hbx.x + hbx.width), (int) (hbx.y + hbx.height)).isSolid())
            return false;
        return true;
    }
    ...
    /**
     * Force the player from stopping it's movement. This will also prevent the polayer from sprinting util he presses
     * the sprint key again.
     */
    ...
    public void update() {
        if (Persistence.player != null
                && Persistence.player.getTeamLeader().species().getID() != this.playersprite.pointer.data.id)
            this.updateSprite();
