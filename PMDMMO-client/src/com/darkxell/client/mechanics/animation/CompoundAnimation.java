...
public class CompoundAnimation extends PokemonAnimation {
    ...
    public void postrender(Graphics2D g, int width, int height) {
        ...
        for (int i = 0; i < this.components.size(); ++i)
            if (this.tick() >= this.componentDelays.get(i))
                this.components.get(i).render(g, width, height); // Keep render because by default,
                                                                 // PokemonAnimation#render() calls postrender()
    }
    ...
    public void start() {
        super.start();
        for (int i = 0; i < this.components.size(); ++i)
            if (this.componentDelays.get(i) == 0)
    ...
    public void update() {
        ...
        for (int i = 0; i < this.components.size(); ++i)
            if (this.tick() == this.componentDelays.get(i))
                this.components.get(i).start();
            else if (this.tick() > this.componentDelays.get(i))
